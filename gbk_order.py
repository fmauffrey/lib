...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Re-order Genbank files from the same gene.")

    # Add subparser to allow the different commands
    subparser = parser.add_subparsers(dest='command')
    find_parser = subparser.add_parser('find', description = "Find genes common to all Genbank files.")
    order_parser = subparser.add_parser('order', description = "Re-order Genbank files.")
    extract_parser = subparser.add_parser('extract', description = "Extract genes surrounding a targeted gene")
    ...
    if args.command not in ["find", "order", "extract"]:
        sys.exit("Commands available: find, order")
